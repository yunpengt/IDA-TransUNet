...
class Encoder(nn.Module):
    ...
    def forward(self, hidden_states,fast_weights):
        ...
        fuse_attention=self.conv1x1(fuse_attention,fast_weights,'transformer.encoder.conv1x1')
        fuse_attention=fuse_attention.squeeze(1)
        fuse_attention = self.encoder_norm(fuse_attention,fast_weights,'transformer.encoder.encoder_norm')
        encoded = self.encoder_norm(hidden_states,fast_weights,'transformer.encoder.encoder_norm')
        return fuse_attention,encoded
...
class Transformer(nn.Module):
    ...
    def forward(self, input_ids,fast_weights=None):
        ...
        fuse,encoded= self.encoder(embedding_output,fast_weights)  # (B, n_patch, hidden)
        return fuse,encoded, features
...
class VisionTransformer(nn.Module):
    def __init__(self, config, img_size=224, num_classes=21843, zero_head=False,weights=None):
        ...
        self.fc3 = Linear(140,14)
      
    def forward(self, x,fast_weights=None):
        if x.size()[1] == 1:
            x = x.repeat(1,3,1,1)
      
        x,encoded, features = self.transformer(x,fast_weights)
        c=x.flatten(1)
        c=F.relu(self.fc1(c,fast_weights,'fc1'))
        c=F.relu(self.fc2(c,fast_weights,'fc2'))
        c=self.fc3(c,fast_weights,'fc3')
        x = self.decoder(x,features,fast_weights)
