...
class BatchNorm2d(nn.BatchNorm2d):
    def forward(self, x, fast_weights, layer_id):
        if fast_weights == None:
            ...
            return F.batch_norm(x, self.running_mean, self.running_var, weight_adapt, bias_adapt, self.training,
                                self.momentum, self.eps)
...
class Embeddings(nn.Module):
    ...
    def forward(self, x, fast_weights=None):
        ...
        x = self.patch_embeddings(x, fast_weights,
                                  'transformer.embeddings.patch_embeddings')  # (B, hidden. n_patches^(1/2), n_patches^(1/2))
        x = x.flatten(2)
...
class VisionTransformer(nn.Module):
    def __init__(self, config, img_size=224, num_classes=21843, zero_head=False, weights=None):
        ...
        self.fc2 = Linear(388, 194)
        self.fc3 = Linear(194, 70)
    ...
    def forward(self, x, fast_weights=None):
        ...
        x, attn_weights, features = self.transformer(x, fast_weights)
    ...
    def load_from(self, weights):
        with torch.no_grad():
            ...
            if self.transformer.embeddings.hybrid:
                self.transformer.embeddings.hybrid_model.stdconv2d.weight.copy_(
